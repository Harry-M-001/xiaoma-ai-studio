...
## 12. 补帧：这一版我们**没有**下发模板（是故意的）

「补帧」那个功能这一版只做本机（RIFE），走 ComfyUI 那条**没接线**。
原因有两条，都写在这里，免得以后有人以为是漏了：

1. **要对它传视频**，而现在的 ComfyUI 适配器只做图片上传。补帧的工作流要吃一段片子，
   这条链路得先补上；
2. **社区那套补帧节点要用户自己装**（`ComfyUI-Frame-Interpolation`，MIT，含 RIFE /
   FILM / GMFSS / IFRNet 等十几种，配合 `VideoHelperSuite` 才能读视频）。
   模板里的节点名与连线**没法在我们这儿验证**——发一份没验过的模板，
   用户拿到的是「导入就报错」，比不发更糟。

想自己走这条路的话：在 ComfyUI 里装上面那两个节点包，然后在**画布**上连一次
（`Load Video → Frame Interpolation（选 RIFE）→ Video Combine`），
上传成工作流即可当普通工作流跑。我们这边不用改任何东西。

**本机那条已经能用**，而且在「本机引擎」页装好 RIFE 之后就能选目标帧率
（24 → 60 这种非整数倍也行，但只有 `rife-v4` / `rife-v4.6` 支持，
其余模型只做 2 倍——界面上会写明）。
